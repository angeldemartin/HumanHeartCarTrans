...
## subset umap healthy
```{r subset umap healthy}
seuratFbHealthy <- subset(seuratAllFb, diseaseCond == "healthy")

Idents(seuratFbHealthy) <- seuratFbHealthy$clusterNameAllFb
DimPlot(seuratFbHealthy, reduction = "umap", pt.size = 0.5, cols = colAllFb, label = TRUE)
DimPlot(seuratFbHealthy, reduction = "umap", pt.size = 0.5, cols = colAllFb)
DimPlot(seuratFbHealthy, reduction = "umap", pt.size = 0.5, cols = colAllFb)  + theme(legend.position = "null")

Idents(seuratFbHealthy) <- seuratFbHealthy$diseaseCond
DimPlot(seuratFbHealthy, reduction = "umap", pt.size = 0.5, cols = coldiseaseCond)
DimPlot(seuratFbHealthy, reduction = "umap", pt.size = 0.5, cols = coldiseaseCond) + theme(legend.position = "null")
```

## subset umap explant
```{r subset umap explant}
seuratFbExplant <- subset(seuratAllFb, diseaseCond == "explant")

Idents(seuratFbExplant) <- seuratFbExplant$clusterNameAllFb
DimPlot(seuratFbExplant, reduction = "umap", pt.size = 0.5, cols = colAllFb, label = TRUE)
DimPlot(seuratFbExplant, reduction = "umap", pt.size = 0.5, cols = colAllFb)
DimPlot(seuratFbExplant, reduction = "umap", pt.size = 0.5, cols = colAllFb)  + theme(legend.position = "null")

Idents(seuratFbExplant) <- seuratFbExplant$diseaseCond
DimPlot(seuratFbExplant, reduction = "umap", pt.size = 0.5, cols = coldiseaseCond)
DimPlot(seuratFbExplant, reduction = "umap", pt.size = 0.5, cols = coldiseaseCond) + theme(legend.position = "null")
```

## subset umap Visit1
```{r subset umap Visit1}
seuratFbVisit1 <- subset(seuratAllFb, diseaseCond == "visit1")

Idents(seuratFbVisit1) <- seuratFbVisit1$clusterNameAllFb
DimPlot(seuratFbVisit1, reduction = "umap", pt.size = 0.5, cols = colAllFb, label = TRUE)
DimPlot(seuratFbVisit1, reduction = "umap", pt.size = 0.5, cols = colAllFb)
DimPlot(seuratFbVisit1, reduction = "umap", pt.size = 0.5, cols = colAllFb)  + theme(legend.position = "null")

Idents(seuratFbVisit1) <- seuratFbVisit1$diseaseCond
DimPlot(seuratFbVisit1, reduction = "umap", pt.size = 0.5, cols = coldiseaseCond)
DimPlot(seuratFbVisit1, reduction = "umap", pt.size = 0.5, cols = coldiseaseCond) + theme(legend.position = "null")
```

## subset umap Visit2
```{r subset umap Visit2}
seuratFbVisit2 <- subset(seuratAllFb, diseaseCond == "visit2")

Idents(seuratFbVisit2) <- seuratFbVisit2$clusterNameAllFb
DimPlot(seuratFbVisit2, reduction = "umap", pt.size = 0.5, cols = colAllFb, label = TRUE)
DimPlot(seuratFbVisit2, reduction = "umap", pt.size = 0.5, cols = colAllFb)
DimPlot(seuratFbVisit2, reduction = "umap", pt.size = 0.5, cols = colAllFb)  + theme(legend.position = "null")

Idents(seuratFbVisit2) <- seuratFbVisit2$diseaseCond
DimPlot(seuratFbVisit2, reduction = "umap", pt.size = 0.5, cols = coldiseaseCond)
DimPlot(seuratFbVisit2, reduction = "umap", pt.size = 0.5, cols = coldiseaseCond) + theme(legend.position = "null")
```

## subset umap Visit3
```{r subset umap Visit3}
seuratFbVisit3 <- subset(seuratAllFb, diseaseCond == "visit3")

Idents(seuratFbVisit3) <- seuratFbVisit3$clusterNameAllFb
DimPlot(seuratFbVisit3, reduction = "umap", pt.size = 0.5, cols = colAllFb, label = TRUE)
DimPlot(seuratFbVisit3, reduction = "umap", pt.size = 0.5, cols = colAllFb)
DimPlot(seuratFbVisit3, reduction = "umap", pt.size = 0.5, cols = colAllFb)  + theme(legend.position = "null")

Idents(seuratFbVisit3) <- seuratFbVisit3$diseaseCond
DimPlot(seuratFbVisit3, reduction = "umap", pt.size = 0.5, cols = coldiseaseCond)
DimPlot(seuratFbVisit3, reduction = "umap", pt.size = 0.5, cols = coldiseaseCond) + theme(legend.position = "null")
```

## subset umap Visit1to3
```{r subset umap Visit1to3}
seuratFbVisit1to3 <- subset(seuratAllFb, diseaseCond %in% c("visit1","visit2","visit3"))

Idents(seuratFbVisit1to3) <- seuratFbVisit1to3$clusterNameAllFb
DimPlot(seuratFbVisit1to3, reduction = "umap", pt.size = 0.5, cols = colAllFb, label = TRUE)
DimPlot(seuratFbVisit1to3, reduction = "umap", pt.size = 0.5, cols = colAllFb)
DimPlot(seuratFbVisit1to3, reduction = "umap", pt.size = 0.5, cols = colAllFb)  + theme(legend.position = "null")

Idents(seuratFbVisit1to3) <- seuratFbVisit1to3$diseaseCond
DimPlot(seuratFbVisit1to3, reduction = "umap", pt.size = 0.5, cols = coldiseaseCond)
DimPlot(seuratFbVisit1to3, reduction = "umap", pt.size = 0.5, cols = coldiseaseCond) + theme(legend.position = "null")
```
...
###subset healthy and eyplant
seuratAllFbhealexpl <- subset(seuratAllFb, diseaseCond %in% c("healthy", "explant"))
table(seuratAllFbhealexpl$diseaseCond)
###diseaseCond
datList <- NULL
for(con in unique(seuratAllFbhealexpl$diseaseCond)){
  seuratSub <- subset(seuratAllFbhealexpl, diseaseCond==con)
  print(dim(seuratSub))
  dat_con <- as.data.frame(table(seuratSub$clusterNameAllFb)) %>%
  mutate(percent=Freq/ncol(seuratSub)) %>% mutate(diseaseCond=con)
  datList[[con]] <- dat_con
...
## plot abundance
ggbarplot(dat_all, x= "diseaseCond", y= "percent", fill = "Var1", palette = colAllFb, legend = "right", legend.titel = "cluster", ylab = "frequency")  + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + scale_x_discrete(limits=orddiseaseCond2)
```

## fractions according to patients&disease cond
```{r fractions, fig.height=8, fig.width=10}
##set order
seuratAllFb$diseaseCond <- factor(seuratAllFb$diseaseCond, levels=c("healthy", "explant", "visit1", "visit2", "visit3"))

## 1. create data.frame with cluster counts per patient
## change "RNA_snn_res.0.25" to subset/cluster you're interested in ...
datFrac <- data.frame(table(seuratAllFb$patient_diseaseCond, seuratAllFb$clusterNameAllFb))
...
datSumPat <- data.frame(table(seuratAllFb$patient_diseaseCond, seuratAllFb$diseaseCond)) %>% 
  filter(Freq >0)
...
createClusterPlot <- function(cluster) {
  ...
  ggbarplot(datFracSumC, x = "diseaseCond", y = "relCnt",
            ...
            add.params = list(color = "black", fill = "diseaseCond",size = 2),
            size = 5,
            position = position_dodge(0.9),
            xlab = cluster,
            ylab = "relative abundance",
...
## plot boxplot with abundances for individual subsets
clusterVec <- levels(seuratAllFb)
createClusterPlot <- function(cluster) {
  datFracSumC <- datFracSum %>% filter(subset == cluster)
  ggboxplot(datFracSumC, x = "diseaseCond", y = "relCnt", color = "diseaseCond", palette = coldiseaseCond ,add = "jitter", 
            add.params = list(shape = 21,        # Use shape 21 for fill + outline control
                            color = "black",   # Black outline for points
                            fill = "diseaseCond", # Fill based on diseaseCond
                            size = 4)) +
    ggtitle(cluster) +
    stat_compare_means(method = "kruskal.test", label.y = 0.0)
}
...
```{r heatmap chemkines cytokine, fig.height=10, fig.width=8}
Idents(seuratAllFb) <- seuratAllFb$clusterNameAllFb
levels(seuratAllFb)
...
genesPlot <- data.frame(gene=c("CCL2","CCL11" ,"CCL19", "CCL21", "CXCL14", "CXCL10", "CXCL1", "CXCL9", "CXCL12", "CXCL2", "IL6R", "IL32", "IL15RA","IL4R","IL10RA", "IL15", "IL1R1", "IL11RA","IL16","IL33", "IL27RA", "IL17D", "IL6", "IL1RL1"))
...
## subset healthy and convert seurat object to sce object
seuratAllFbheal <- subset(seuratAllFb, diseaseCond == "healthy")
table(seuratAllFbheal$diseaseCond)
sceheal <- as.SingleCellExperiment(seuratAllFbheal)

## subset visit1 and convert seurat object to sce object
seuratAllFbvisit1 <- subset(seuratAllFb, diseaseCond == "visit1")
table(seuratAllFbvisit1$diseaseCond)
scevisit1 <- as.SingleCellExperiment(seuratAllFbvisit1)

## subset visit2 and convert seurat object to sce object
seuratAllFbvisit2 <- subset(seuratAllFb, diseaseCond == "visit2")
table(seuratAllFbvisit2$diseaseCond)
scevisit2 <- as.SingleCellExperiment(seuratAllFbvisit2)

## subset visit3 and convert seurat object to sce object
seuratAllFbvisit3 <- subset(seuratAllFb, diseaseCond == "visit3")
table(seuratAllFbvisit3$diseaseCond)
scevisit3 <- as.SingleCellExperiment(seuratAllFbvisit3)

## subset explant and convert seurat object to sce object
seuratAllFbexplant <- subset(seuratAllFb, diseaseCond == "explant")
table(seuratAllFbexplant$diseaseCond)
sceexplant <- as.SingleCellExperiment(seuratAllFbexplant)
```
...
##healthy only
scehealSub <- sceheal[which(rownames(sceheal) %in% signGenes$geneID),]
cntMat <- rowSums(t(as.matrix(
    scehealSub@assays@data$logcounts)))/nrow(signGenes)
scehealSub$sign <- cntMat
scehealSub$sign2 <- scehealSub$sign
##check max and min values
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 2))
scehealSub$sign2[which(scehealSub$sign > 2)] <- 2
plotUMAP(scehealSub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(scehealSub, colour_by = "sign2", point_size = 0.5) + sc

##visit1 only
scevisit1Sub <- scevisit1[which(rownames(scevisit1) %in% signGenes$geneID),]
cntMat <- rowSums(t(as.matrix(
    scevisit1Sub@assays@data$logcounts)))/nrow(signGenes)
scevisit1Sub$sign <- cntMat
scevisit1Sub$sign2 <- scevisit1Sub$sign
##check max and min values
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 2))
scevisit1Sub$sign2[which(scevisit1Sub$sign > 2)] <- 2
plotUMAP(scevisit1Sub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(scevisit1Sub, colour_by = "sign2", point_size = 0.5) + sc

##visit2 only
scevisit2Sub <- scevisit2[which(rownames(scevisit2) %in% signGenes$geneID),]
cntMat <- rowSums(t(as.matrix(
    scevisit2Sub@assays@data$logcounts)))/nrow(signGenes)
scevisit2Sub$sign <- cntMat
scevisit2Sub$sign2 <- scevisit2Sub$sign
##check max and min values
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 2))
scevisit2Sub$sign2[which(scevisit2Sub$sign > 2)] <- 2
plotUMAP(scevisit2Sub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(scevisit2Sub, colour_by = "sign2", point_size = 0.5) + sc

##visit3 only
scevisit3Sub <- scevisit3[which(rownames(scevisit3) %in% signGenes$geneID),]
cntMat <- rowSums(t(as.matrix(
    scevisit3Sub@assays@data$logcounts)))/nrow(signGenes)
scevisit3Sub$sign <- cntMat
scevisit3Sub$sign2 <- scevisit3Sub$sign
##check max and min values
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 2))
scevisit3Sub$sign2[which(scevisit3Sub$sign > 2)] <- 2
plotUMAP(scevisit3Sub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(scevisit3Sub, colour_by = "sign2", point_size = 0.5) + sc

##explant only
sceexplantSub <- sceexplant[which(rownames(sceexplant) %in% signGenes$geneID),]
cntMat <- rowSums(t(as.matrix(
    sceexplantSub@assays@data$logcounts)))/nrow(signGenes)
sceexplantSub$sign <- cntMat
sceexplantSub$sign2 <- sceexplantSub$sign
##check max and min values
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 2))
sceexplantSub$sign2[which(sceexplantSub$sign > 2)] <- 2
plotUMAP(sceexplantSub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(sceexplantSub, colour_by = "sign2", point_size = 0.5) + sc
```
...
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 0.5))
scehealSub$sign2[which(scehealSub$sign > 0.5)] <- 0.5
plotUMAP(scehealSub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(scehealSub, colour_by = "sign2", point_size = 0.5) + sc
...
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 0.5))
scevisit1Sub$sign2[which(scevisit1Sub$sign > 0.5)] <- 0.5
plotUMAP(scevisit1Sub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(scevisit1Sub, colour_by = "sign2", point_size = 0.5) + sc
...
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 0.5))
scevisit2Sub$sign2[which(scevisit2Sub$sign > 0.5)] <- 0.5
plotUMAP(scevisit2Sub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(scevisit2Sub, colour_by = "sign2", point_size = 0.5) + sc
...
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 0.5))
scevisit3Sub$sign2[which(scevisit3Sub$sign > 0.5)] <- 0.5
plotUMAP(scevisit3Sub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(scevisit3Sub, colour_by = "sign2", point_size = 0.5) + sc
...
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 0.5))
sceexplantSub$sign2[which(sceexplantSub$sign > 0.5)] <- 0.5
plotUMAP(sceexplantSub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(sceexplantSub, colour_by = "sign2", point_size = 0.5) + sc
```
...
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 1))
scehealSub$sign2[which(scehealSub$sign > 1)] <- 1
plotUMAP(scehealSub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(scehealSub, colour_by = "sign2", point_size = 0.5) + sc
...
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 1))
scevisit1Sub$sign2[which(scevisit1Sub$sign > 1)] <- 1
plotUMAP(scevisit1Sub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(scevisit1Sub, colour_by = "sign2", point_size = 0.5) + sc
...
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 1))
scevisit2Sub$sign2[which(scevisit2Sub$sign > 1)] <- 1
plotUMAP(scevisit2Sub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(scevisit2Sub, colour_by = "sign2", point_size = 0.5) + sc
...
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 1))
scevisit3Sub$sign2[which(scevisit3Sub$sign > 1)] <- 1
plotUMAP(scevisit3Sub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(scevisit3Sub, colour_by = "sign2", point_size = 0.5) + sc
...
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 1))
sceexplantSub$sign2[which(sceexplantSub$sign > 1)] <- 1
plotUMAP(sceexplantSub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(sceexplantSub, colour_by = "sign2", point_size = 0.5) + sc
```
...
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 0.8))
scehealSub$sign2[which(scehealSub$sign > 0.8)] <- 0.8
plotUMAP(scehealSub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(scehealSub, colour_by = "sign2", point_size = 0.5) + sc
...
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 0.8))
scevisit1Sub$sign2[which(scevisit1Sub$sign > 0.8)] <- 0.8
plotUMAP(scevisit1Sub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(scevisit1Sub, colour_by = "sign2", point_size = 0.5) + sc
...
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 0.8))
scevisit2Sub$sign2[which(scevisit2Sub$sign > 0.8)] <- 0.8
plotUMAP(scevisit2Sub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(scevisit2Sub, colour_by = "sign2", point_size = 0.5) + sc
...
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 0.8))
scevisit3Sub$sign2[which(scevisit3Sub$sign > 0.8)] <- 0.8
plotUMAP(scevisit3Sub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(scevisit3Sub, colour_by = "sign2", point_size = 0.5) + sc
...
##max(sceSub$sign)
sc <- scale_colour_gradientn(colours = pal(100), limits=c(0, 0.8))
sceexplantSub$sign2[which(sceexplantSub$sign > 0.8)] <- 0.8
plotUMAP(sceexplantSub, colour_by = "sign2", point_size = 0.5) + sc +
  theme(legend.position = "none")
plotUMAP(sceexplantSub, colour_by = "sign2", point_size = 0.5) + sc
```
...
## feature plots
```{r feature plot}
FeaturePlot(seuratAllFb, features = "ENSG00000108821.COL1A1", pt.size = 0.5, cols = c("lightgrey", "#BE3144"))
```
```{r violin plots AllFb}
Idents(seuratAllFb) <- seuratAllFb$clusterNameAllFb
VlnPlot(object=seuratAllFb, features = "ENSG00000108821.COL1A1", pt.size = 0, cols = colAllFb)
```
